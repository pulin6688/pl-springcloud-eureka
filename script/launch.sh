...
CONFIGS_PATHNAME=configs-${FILENAME}
CURR=`date +%Y%m%d`
[[ -z "$JAVA_OPTS" ]] && JAVA_OPTS=" -server -Xms2048M -Xmx2048M  -XX:MaxMetaspaceSize=128M -Xss256K -XX:+PrintClassHistogram -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintHeapAtGC -Xloggc:./GC_${FILENAME}_${CURR}.log  -Djava.awt.headless=true "


if [ $ENVS == "dev" ];then
    JAVA_OPTS=" -server -Xms256M -Xmx512M  -XX:MaxMetaspaceSize=128M -Xss256K -XX:+PrintClassHistogram -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintHeapAtGC -Xloggc:./GC_${FILENAME}_${CURR}.log  -Djava.awt.headless=true "
fi
...
un_zip(){
    if [ $FORCE_UNZIP_ONEAPM = "true" ] || [ ! -d ${ONEAPM_PATHNAME} ] ; then
        ...
        unzip -o $0 "OneAPM/*" -d  ${ONEAPM_PATHNAME}

 	    rm -rf ${CONFIGS_PATHNAME}
        unzip -o $0 "configs/${ENVS}/*" -d  ${CONFIGS_PATHNAME}
        cp ${CONFIGS_PATHNAME}/configs/${ENVS}/oneapm.properties ${ONEAPM_PATHNAME}/OneAPM
    fi
}
...
#command="$javaexe -javaagent:${WORKING_DIR}/${ONEAPM_PATHNAME}/OneAPM/oneapm.jar -Dsun.misc.URLClassPath.disableJarChecking=true $JAVA_OPTS -jar -Dspring.config.location=${CONFIGS_PATHNAME}/configs/${ENVS}/bootstrap.yml $jarfile $RUN_ARGS $*"
if [ $ENVS ];then
 command="$javaexe -javaagent:${WORKING_DIR}/${ONEAPM_PATHNAME}/OneAPM/oneapm.jar -Dsun.misc.URLClassPath.disableJarChecking=true $JAVA_OPTS -jar -Dspring.config.location=${CONFIGS_PATHNAME}/configs/${ENVS}/bootstrap.yml $jarfile $RUN_ARGS $*"
else
